...
// Copyright (c) 2019-2020, BitcoinPhantom Developers
...
namespace CryptoNote
{
    namespace parameters
    {
        ...
        const uint64_t CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX = 413060048;
        ...
        const uint64_t GENESIS_BLOCK_TIMESTAMP = 1580490603;
        ...
        /* TODO: Remove? */
        const uint64_t MINIMUM_FEE = UINT64_C(8);
         ...
         * - 7 BTCXP vs 7.12 BTCXP. You can read this as.. the fee per chunk
         * is 700 atomic units. The fee per byte is 700 / chunk size. */
        const double MINIMUM_FEE_PER_BYTE_V1 = 700.00 / FEE_PER_BYTE_CHUNK_SIZE;
        ...
        const uint64_t MAXIMUM_MIXIN_V1 = 80;
        ...
        /* 45 trillion atomic, or 450 billion BTCXP -> Max supply / mixin+1 outputs */
        /* This is enforced on the daemon side. An output > 450 billion causes
         * an invalid block. */
        const uint64_t MAX_OUTPUT_SIZE_NODE   = 450'000'000'000'00;

        /* 1000 billion atomic, or 10 billion BTCXP */
        /* This is enforced on the client side. An output > 10 billion will not
         * be created in a transaction */
        const uint64_t MAX_OUTPUT_SIZE_CLIENT = 10'000'000'000'00;
        ...
        const uint32_t UPGRADE_HEIGHT_V4 = 370000; // Upgrade height for CN-Lite Variant 1 switch.

        const uint32_t UPGRADE_HEIGHT_V5 = 1400000; // Upgrade height for CN-Turtle Variant 2 switch.

        const uint32_t UPGRADE_HEIGHT_V6 = 1600000; // Upgrade height for Chukwa switch.
    ...
    const char CRYPTONOTE_NAME[] = "BitcoinPhantom";
    ...
    const uint32_t P2P_DEFAULT_HANDSHAKE_INTERVAL = 20; // seconds
    const uint32_t P2P_DEFAULT_PACKET_MAX_SIZE = 50000000; // 50000000 bytes maximum packet size
    const uint32_t P2P_DEFAULT_PEERS_IN_HANDSHAKE = 85;
    ...
    const char LATEST_VERSION_URL[] = "http://www.bitcoinphantomcurrency.com";

    const std::string LICENSE_URL = "";

    const static boost::uuids::uuid CRYPTONOTE_NETWORK = {
        {0xb3, 0x0d, 0x2a, 0x4c, 0xcb, 0x32, 0x67, 0x21, 0x43, 0xf7, 0x51, 0xa6, 0xb4, 0xc3, 0x23, 0xe7}};
